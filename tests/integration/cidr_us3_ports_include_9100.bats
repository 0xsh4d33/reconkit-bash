...
@test "printer exclusion applies when selected ports include 9100" {
  ...
  END_SCANNER_HTTPX_FIXTURE="$REPO_ROOT/tests/fixtures/httpx/cidr-printer-exclusion-web.jsonl" \
    run_cidr_scanner --cidr 192.0.2.0/29 --ports 80,9100 --output "$output" --log "$log" --tmp-dir "$BATS_TEST_TMPDIR"
